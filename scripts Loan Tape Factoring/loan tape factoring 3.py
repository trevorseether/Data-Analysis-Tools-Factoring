...
SELECT a.code as loan_id,
	...
	CASE
		WHEN a.currency_auctions = 'USD' THEN a.total_net_amount_pending_payment * a.exchange_rate 
		WHEN a.currency_auctions = 'USD' THEN a.total_net_amount_pending_payment * a.exchange_rate 
	...
	(CASE 
	WHEN (g.product = 'factoring') and (a.currency_auctions = 'PEN') THEN g.proforma_simulation_financing_advance
	WHEN (g.product = 'factoring') and (a.currency_auctions = 'USD') THEN g.proforma_simulation_financing_advance * a.exchange_rate
	WHEN (g.product != 'factoring') and (a.currency_auctions = 'PEN') THEN g.invoice_net_amount
	WHEN (g.product != 'factoring') and (a.currency_auctions = 'USD') THEN g.invoice_net_amount * a.exchange_rate
...
df['Financing Percentage'] = df['Financing Percentage'].clip(upper = 1)

#%% porcentaje de Avance
df['Percentage of advance'] = df['Monto Adelantado'] / df['principal_amount']

print('revisar estos 2 casos, tienen el monto de adelanto mayor al loan, probablemente la moneda está mal')
df = df[df['Percentage of advance'] <= 1]
...
df['principal_amount_2']  = '=+V2*U2'

df['Financing Cost']    = '=W2*((1+X2)^(P2/30)-1)'

df['Advance Amount']   = '=+Z2*W2'

df['Structuring Fee']   = '=W2-AA2'

df['Warranty']          = '.=MAX(SI(J2="confirming";0;U2-AA2-AB2-Y2);0)'
...
'Percentage of advance','Advance Amount','Structuring Fee','Warranty','Repayment Amount',
...
'fee_outstanding','penalty_outstanding','days_past_due','collateral_description', 'principal_amount_2']
...
destino = f"{cierre}_Loan Tape Document For Alt Lenders Factoring Mb {hoy_formateado} 2.xlsx"
