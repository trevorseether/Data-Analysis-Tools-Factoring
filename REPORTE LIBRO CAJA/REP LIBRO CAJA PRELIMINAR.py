...
hoy_formateado = datetime.today().strftime('%Y-%m-%d')  # o '%Y-%m-%d', etc.
tc = 3.5

#%%
def proceso_completo(ubi, nombre_archivo, tc, sheets_objetivo):
    # columnas:
    columnas = [
        'SD.',
        'Comprobante',
    ...
    for archivo in sorted(os.listdir(ubi)):
        ...
        if (archivo.endswith('.xlsx')
            ...
            and not archivo.startswith('datos_concatenados_')
        ):
...
#%%
    ...
    mask_ingreso = pd.to_numeric(df_final['Ingreso'], errors='coerce').isna() & df_final['Ingreso'].notna()
    mask_egreso  = pd.to_numeric(df_final['Egreso'],  errors='coerce').isna() & df_final['Egreso'].notna()
    df_errores_detalle = df_final.copy()
    df_errores_detalle['Ingreso_error'] = mask_ingreso
    df_errores_detalle['Egreso_error'] = mask_egreso
    df_errores_detalle = df_errores_detalle[df_errores_detalle['Ingreso_error'] | df_errores_detalle['Egreso_error']]
    if df_errores_detalle.shape[0] > 0:
        df_errores_detalle.to_excel(fr'{ubi}\errores.xlsx', index = False)
        print(fr'Creadoo: {ubi}\errores.xlsx')
...
#%%
    os.chdir(ubi)
    df_final.to_excel(f'datos_concatenados_{nombre_archivo} {hoy_formateado}.xlsx', index = False)
    '''AQUÍ TEMRINA LA FUNCIÓN'''
...
###############################################################################
"PROCESO PARA P2P"
ubi            = r'C:\Users\Joseph Montoya\Desktop\REPORTE LIBRO CAJA\REPORTE P2P'
nombre_archivo = 'Reporte P2P'
sheets_objetivo = ['BCP MN', 'BCP MN CAJA', 'BCP ME CAJA', 'BCP ME']
proceso_completo(ubi, nombre_archivo, tc, sheets_objetivo)
###############################################################################
"PROCESO PARA TANDIA"
