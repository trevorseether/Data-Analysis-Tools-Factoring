...
, cartera_1 AS (
   ...
   , CAST(CAST(CAST(REPLACE(DP.company_ruc, '.com', '') AS DOUBLE) AS BIGINT) AS VARCHAR) provider_ruc
   ...
   , (CASE WHEN (dp."MES CANCELACION" = cm.fecha_eomonth) THEN 0 WHEN (NCPT."NUEVO_CAPITAL" IS NOT NULL) THEN NCPT."NUEVO_CAPITAL" ELSE round(dp.monto_financiado, 2) END) remaining_capital
