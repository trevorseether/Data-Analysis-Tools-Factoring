...
, nuevo_capital_pagos_facturas AS (
   SELECT
     c_digo_de_subasta "code_PF"
   , DATE_FORMAT(fecha_de_pago_registrado, '%Y-%m') "anio_mes"
   , MAX_BY(nuevo_capital___offline, fecha_de_pago_registrado) "NUEVO_CAPITAL"
   , MAX(fecha_de_pago_registrado) "FECHA PAGO"
   , date_add('day', -1, date_add('month', 1, date_trunc('month', MAX(fecha_de_pago_registrado)))) "eomonth"
   FROM
     prod_datalake_master."hubspot__pagos_facturas"
   WHERE ((hs_pipeline = '42484623') AND (nuevo_capital___offline IS NOT NULL))
   GROUP BY c_digo_de_subasta, DATE_FORMAT(fecha_de_pago_registrado, '%Y-%m')
) 
, datos_planos AS (
   SELECT
   ...
   , (CAST(REPLACE(REPLACE(hubspot__deal.tasa_de_financiamiento____, '%', ''), ',', '') AS DOUBLE) / 100) "tasa_de_financiamiento_asignada"
   ...
   , COALESCE(DATE(fac_disb.fecha_de_desembolso_proveedor), DATE(pagos_fact.Fecha_Minima_de_Desembolso), DATE(fecha_de_desembolso__factoring_)) "Fecha_de_desembolso_hub"
   , ((DATE_TRUNC('month', COALESCE(COALESCE(DATE(fac_disb.fecha_de_desembolso_proveedor), DATE(pagos_fact.Fecha_Minima_de_Desembolso), DATE(fecha_de_desembolso__factoring_)), CAST((hubspot__deal.closedate - INTERVAL  '5' HOUR) AS DATE))) + INTERVAL  '1' MONTH) - INTERVAL  '1' DAY) "MES_DESEMBOLSO"
   , (CASE WHEN (hubspot__deal.flag_comprado = 'Si') THEN 'COMPRADO INTERNAMENTE' ELSE 'OP OFFLINE' END) flag_es_offline
   , COALESCE(FC1."Fecha de Cierre Final", FC2.FECHA_PAGO_HUB_TICKETS) "FECHA CANCELACION"
   , ((date_trunc('month', COALESCE(FC1."Fecha de Cierre Final", FC2.FECHA_PAGO_HUB_TICKETS)) + INTERVAL  '1' MONTH) - INTERVAL  '1' DAY) "MES CANCELACION"
...
, cartera_auxiliar AS (
   ...
   FROM
     (cortes_mensuales cm
   LEFT JOIN datos_planos dp ON ((dp.MES_DESEMBOLSO <= cm.fecha_eomonth) AND (cm.fecha_eomonth <= COALESCE(dp."MES CANCELACION", DATE_ADD('day', -1, DATE_ADD('month', 1, DATE_TRUNC('month', current_date)))))))
) 
, nuevo_capital_propagado_tabla AS (
   SELECT
     c.code
   , c.fecha_cierre
   , MAX_BY(ncpf.nuevo_capital, ncpf."FECHA PAGO") "NUEVO_CAPITAL"
   FROM
     (cartera_auxiliar c
   LEFT JOIN nuevo_capital_pagos_facturas ncpf ON ((ncpf.code_pf = c.code) AND (ncpf.eomonth <= c.fecha_cierre)))
...
, cartera_1 AS (
   ...
   , (CASE WHEN (CM.fecha_eomonth = DP."MES CANCELACION") THEN 0 WHEN (ncpt."NUEVO_CAPITAL" IS NOT NULL) THEN ncpt."NUEVO_CAPITAL" ELSE dp.monto_neto_de_facturas__factoring_ END) total_net_amount_pending_payment
   ...
   , (CASE WHEN (DATE_FORMAT(DP."FECHA CANCELACION", '%Y%m') = DATE_FORMAT(cm.fecha_eomonth, '%Y%m')) THEN DP."FECHA CANCELACION" END) payment_date
   ...
   , (CASE WHEN (CAST(cm.fecha_eomonth AS date) = CAST(DP.MES_DESEMBOLSO AS date)) THEN 1 ELSE 0 END) q_desembolso
   , (CASE WHEN (CAST(cm.fecha_eomonth AS date) = CAST(DP.MES_DESEMBOLSO AS date)) THEN DP.monto_financiado ELSE 0 END) m_desembolso
   , 0 new_clients
   , 0 recurrent_clients
   , 0 new_providers
   ...
   LEFT JOIN nuevo_capital_propagado_tabla NCPT ON ((NCPT.CODE = DP.CODE) AND (CAST(NCPT.fecha_cierre AS DATE) = CAST(cm.fecha_eomonth AS DATE))))
) 
SELECT DISTINCT
  c1.*
...
WHERE (NOT (c1.code IN ('prueba123')))
