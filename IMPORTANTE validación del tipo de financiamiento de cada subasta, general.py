...
hubspots['monto_financiado'] = hubspots['monto_financiado'].fillna(1.0)

#%% dando estructura a los del hubpot
hubspots['CROWD'] = np.where(hubspots['fuente_fondeo'] == 'PLATAFORMA',
                             hubspots['monto_financiado'],
                             0)
hubspots['GESTORA'] = np.where( ~hubspots['fuente_fondeo'].isin( ['PLATAFORMA', 'INTERNO', 'FONDOS PROPIOS - FACTORING'] ),
                                hubspots['monto_financiado'],
                                0)
hubspots['ONBALANCE'] = np.where(hubspots['fuente_fondeo'].isin( ['INTERNO', 'FONDOS PROPIOS - FACTORING'] ),
                                 hubspots['monto_financiado'],
                                 0)
hubspots['CODE'] = hubspots['dealname'].copy()
#%%%
...
# cartera
data = pd.read_excel(r'G:/.shortcut-targets-by-id/1alT0hxGsi0dfv0NYh_LB4NrT2tKEgPK8/Cierre Factoring/Reportes/Inputs/DATA portafolio factoring (202602) 11-03-2026.xlsx',
...
# cosecha propia
cosecha = pd.read_excel(r'G:/.shortcut-targets-by-id/1alT0hxGsi0dfv0NYh_LB4NrT2tKEgPK8/Cierre Factoring/Reportes/Inputs/DATA cosecha factoring (202602) 11-03-2026.xlsx')
...
hubspot_final = hubspots[['CODE', 'CROWD', 'GESTORA', 'ONBALANCE']]
proporciones = pd.concat([df_final, hubspot_final], ignore_index= True)
